...
async function loadData() {
  ...
  loading.value = true
  try {
    const data = await customerApi.changes(props.customerId, 100)
...
watch(() => props.customerId, (id) => { if (id) loadData() }, { immediate: true })

function colorOf(field: string): 'primary' | 'success' | 'warning' | 'danger' {
  if (field === 'merge_in') return 'success'
  if (field === 'merged_to') return 'warning'
  if (field.includes('delete')) return 'danger'
  return 'primary'
}
</script>
